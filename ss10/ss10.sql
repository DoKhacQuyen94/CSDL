...
values ('nguyenmoi', 'Nguyễn Văn Mới', 'nguyenmoi@test.com', '123456', 'Nam', 'Hà Nội');
select * from view_users_firstname;

delete from users where username = 'nguyenmoi';
select * from view_users_firstname;
...
select u.user_id, u.username, u.full_name, v.total_posts, v.total_comments
...
select 
    ...
    (select count(*) from friends 
     where (friends.user_id = users.user_id or friends.friend_id = users.user_id) 
     and friends.status = 'accepted') as total_friends
from users;
